<script lang="ts">
	...
	export let draggedItem: any;

	function onDelete() {
		writeableVar.set($writeableVar.filter((i: any) => i !== $writeableVar[index]));
	}
</script>

<div class={`m-2 flex ${draggedItem && draggedItem === $writeableVar[index] ? 'opacity-50' : ''}`}>
